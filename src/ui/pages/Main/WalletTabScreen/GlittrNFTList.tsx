...
export function GlittrNFTList() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter only NFT assets (those with asset_image)
      const userNFTs = await Promise.all(Object.entries(_balance.balance.summarized)
        .filter(([key]) => {
        ...
        .map(async ([key, amount]) => {
          ...
          try {
            ...
            console.error(e);
            // const blob = new Blob([Buffer.from(contractInfo.asset_image)], { type: "image/bmp" });
